...
import {
	...
	loadSampleRequest,
	type RiverSolveRequest,
	type RiverSolveResponse,
	solveRiverSpot,
...
const SMOKE_ENABLED = import.meta.env.VITE_POKIE_E2E === "1";
...
export default function App() {
	const [request, setRequest] = useState<RiverSolveRequest>(
		emptyRiverSolveRequest(),
	);
	...
	useEffect(() => {
		loadSampleRequest()
			...
			.catch((nextError) => {
				...
				setStatus(
					"Could not load the backend sample spot. The form is ready for manual input.",
				);
	...
	function updateField<Key extends keyof RiverSolveRequest>(
		key: Key,
		value: RiverSolveRequest[Key],
	) {
		setRequest((current) => ({ ...current, [key]: value }));
	}

	const applySolveResponse = useCallback(
		(solveResponse: RiverSolveResponse) => {
			setValidation({
	...
	useEffect(() => {
		if (!SMOKE_ENABLED || smokeStarted || !sampleReady) {
			return;
		}
		...
		void runSmokeFlow();
	}, [applySolveResponse, request, sampleReady, smokeStarted]);
		...
		<main className="app-shell">
			<section className="hero card">
				<div>
					...
					<p className="lede">
						This app is intentionally narrow: it validates and solves the
						current exact heads-up NLHE river slice through the app-api
						boundary.
					</p>
				</div>
					...
					<div className="two-column">
						<label className="field">
							...
							<textarea
								...
								onChange={(event) =>
									updateField("oopRange", event.target.value)
								}
							/>
						</label>
						<label className="field">
						...
						<label className="field">
							...
							<select
								...
								onChange={(event) =>
									updateField(
										"playerToAct",
										event.target.value as RiverSolveRequest["playerToAct"],
									)
								}
							>
				...
				<section className="stack">
					...
					<section className="card">
						...
						{validation ? (
							...
							<p className="placeholder">
								Validate a request to inspect the canonicalized backend config.
							</p>
						)}
					</section>
					...
					<section className="card">
						...
						{result ? (
							...
							<p className="placeholder">
								Solve a validated request to inspect root EV, exploitability,
								and root infoset strategies.
							</p>
						)}
					</section>
				</section>
