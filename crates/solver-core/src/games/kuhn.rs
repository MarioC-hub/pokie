...
pub fn reference_equilibrium(game: &ExtensiveGame) -> StrategyProfile {
    ...
    probabilities.insert(
        "kuhn:P1:after_check:J".to_string(),
        vec![2.0 / 3.0, 1.0 / 3.0],
    );
    ...
    probabilities.insert(
        "kuhn:P0:after_check_bet:Q".to_string(),
        vec![1.0 / 3.0, 2.0 / 3.0],
    );
...
impl KuhnBuilder {
    ...
    fn build_p0_initial(&mut self, card_p0: Card, card_p1: Card) -> usize {
        let infoset = self.infoset_id(
            Player::P0,
            format!("kuhn:P0:init:{}", card_p0.label()),
            &["check", "bet"],
        );
    ...
    fn showdown_terminal(
        ...
        let utility_p0 = if card_p0.rank() > card_p1.rank() {
            stakes
        } else {
            -stakes
        };
