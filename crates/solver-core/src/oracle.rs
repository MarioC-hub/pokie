...
pub fn exact_exploitability(
    ...
    let worst_case_value_p0_against_p1_best_response =
        pure_best_response_value_p0(game, profile, Player::P1);
...
fn enumerate_best_response(
    ...
    if depth == infosets.len() {
        let value = expected_value_with_pure_response_p0(
            game,
            game.root(),
            profile,
            responding_player,
...
fn expected_value_node_p0(game: &ExtensiveGame, node_id: usize, profile: &StrategyProfile) -> f64 {
    match &game.node(node_id).kind {
        ...
        NodeKind::Chance { outcomes } => outcomes
            .iter()
            .map(|outcome| {
                outcome.probability * expected_value_node_p0(game, outcome.child, profile)
            })
            .sum(),
        NodeKind::Decision {
            player: _,
            ...
            .map(|(probability, action)| {
                probability * expected_value_node_p0(game, action.child, profile)
            })
            .sum(),
    }
}
